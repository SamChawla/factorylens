...
1. **`capture_demo.py`** — seed fresh telemetry, then walk the dashboard and
   alerts, saving screenshots + a recorded video to `scripts/output/`. These are the
...
```bash
uv run python scripts/capture_demo.py            # seed data + capture
uv run python scripts/capture_demo.py --no-seed  # capture only (data already present)
uv run python scripts/capture_demo.py --headed   # watch the browser drive itself
```
...
`scripts/output/` is gitignored — it's regenerable. Copy any keeper into `docs/` if
